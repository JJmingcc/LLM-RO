"""
Sensitivity Analysis using RODIU_LLM.py
Analyzes the impact of GPU rental cost and error rate thresholds (epsilon)
...
def run_sensitivity_analysis(gpu_cost_scale, psi_error, base_data, time_limit=300, mip_gap=0.01):
    """
    Run optimization with scaled GPU rental cost and error thresholds.

    Args:
        gpu_cost_scale: Scaling factor for GPU rental unit cost (p_c)
    ...
    # Scale GPU rental costs
    data.p_c = base_data.p_c * gpu_cost_scale
    ...
    try:
        ...
        results = extract_metrics(optimizer, solution, data, gpu_cost_scale, psi_error, solve_time)
        return results

    except Exception as e:
        print(f"Error for gpu_cost_scale={gpu_cost_scale}, psi_error={psi_error}: {str(e)}")
        return {
            'gpu_cost_scale': gpu_cost_scale,
            'psi_error': psi_error,
            'status': 'ERROR',
...
def extract_metrics(optimizer, solution, data, gpu_cost_scale, psi_error, solve_time):
    """
    ...
    Args:
        ...
        gpu_cost_scale: GPU cost scaling factor
    ...
    return {
        ...
        'scaled_gpu_cost_mean': np.mean(data.p_c),
...
def main():
    ...
    print("="*80)
    print("SENSITIVITY ANALYSIS: GPU RENTAL COST vs ERROR THRESHOLD")
    print("="*80)
    ...
    gpu_cost_scale_values = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5]
    ...
    print(f"\nGPU cost scaling factors: {gpu_cost_scale_values}")
    print(f"Error scaling factors (psi_error): {psi_error_values}")
    print(f"Total scenarios: {len(gpu_cost_scale_values)} × {len(psi_error_values)} = {len(gpu_cost_scale_values) * len(psi_error_values)}")

    # Run sensitivity analysis
    results_list = []
    counter = 0
    total = len(gpu_cost_scale_values) * len(psi_error_values)

    print("\n" + "="*80)
    print("Running sensitivity analysis...")
    print("="*80)

    for gpu_cost_scale in gpu_cost_scale_values:
        for psi_error in psi_error_values:
            counter += 1
            print(f"\n[{counter}/{total}] gpu_cost_scale={gpu_cost_scale:.1f}, psi_error={psi_error:.1f}")

            result = run_sensitivity_analysis(gpu_cost_scale, psi_error, base_data, time_limit=300, mip_gap=0.01)
    ...
    if len(df_success) > 0:
        ...
        csv_filename = os.path.join(output_dir, f'sensitivity_gpu_cost_error_{timestamp}.csv')
...
def create_visualizations(df, output_dir, timestamp):
    ...
    # Total Cost Heatmap
    cost_pivot = df.pivot_table(values='total_cost', index='gpu_cost_scale', columns='psi_error')
    ...
    axes[0, 0].set_ylabel('GPU Cost Scaling')

    # Rental Cost Heatmap
    rental_pivot = df.pivot_table(values='C1_rental', index='gpu_cost_scale', columns='psi_error')
    ...
    axes[0, 1].set_ylabel('GPU Cost Scaling')

    # Unmet Demand Cost Heatmap
    unmet_pivot = df.pivot_table(values='C4_unmet', index='gpu_cost_scale', columns='psi_error')
    ...
    axes[1, 0].set_ylabel('GPU Cost Scaling')

    # Total GPUs Heatmap
    gpu_pivot = df.pivot_table(values='total_gpus', index='gpu_cost_scale', columns='psi_error')
    sns.heatmap(gpu_pivot, annot=True, fmt='.0f', cmap='Greens', ax=axes[1, 1],
                cbar_kws={'label': 'Number of GPUs'})
    axes[1, 1].set_title('Total GPUs Allocated', fontweight='bold', fontsize=12)
    axes[1, 1].set_xlabel('Error Scaling (psi_error)')
    axes[1, 1].set_ylabel('GPU Cost Scaling')

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'heatmaps_gpu_cost_error_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: heatmaps_gpu_cost_error_{timestamp}.png")
    plt.close()

    # Figure 2: Cost Component Breakdown
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))

    # Line plot for fixed GPU cost scaling
    for gpu_scale in sorted(df['gpu_cost_scale'].unique())[::2]:  # Show every other value
        subset = df[df['gpu_cost_scale'] == gpu_scale]
        axes[0].plot(subset['psi_error'], subset['total_cost'],
                    marker='o', label=f'GPU cost scale={gpu_scale:.1f}', linewidth=2)

    axes[0].set_xlabel('Error Scaling (psi_error)', fontsize=11)
    axes[0].set_ylabel('Total Cost ($)', fontsize=11)
    axes[0].set_title('Total Cost vs Error Scaling\n(Different GPU Cost Scalings)', fontweight='bold')
    axes[0].legend(title='GPU Cost Scaling', fontsize=9)
    ...
    for psi_e in sorted(df['psi_error'].unique())[::2]:  # Show every other error value
        ...
        axes[1].plot(subset['gpu_cost_scale'], subset['total_cost'],
                    marker='s', label=f'psi_error={psi_e:.1f}', linewidth=2)

    axes[1].set_xlabel('GPU Cost Scaling', fontsize=11)
    axes[1].set_ylabel('Total Cost ($)', fontsize=11)
    axes[1].set_title('Total Cost vs GPU Cost Scaling\n(Different Error Scalings)', fontweight='bold')
    axes[1].legend(title='Error Scaling', fontsize=9)
    axes[1].grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'cost_trends_gpu_cost_error_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: cost_trends_gpu_cost_error_{timestamp}.png")
    plt.close()
    ...
    x_labels = [f"GPU={row['gpu_cost_scale']:.1f}\nerr={row['psi_error']:.1f}"
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'cost_breakdown_gpu_cost_error_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: cost_breakdown_gpu_cost_error_{timestamp}.png")
    plt.close()
