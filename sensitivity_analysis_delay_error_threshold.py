"""
Sensitivity Analysis for Delay Threshold and Error Rate Threshold
Analyzes the impact of varying delay thresholds (Delta_i) and error rate thresholds (epsilon)
...
def run_sensitivity_analysis(psi_delay, psi_error, base_data, time_limit=300, mip_gap=0.01):
    """
    Run optimization with scaled delay and error rate thresholds.

    Args:
        psi_delay: Scaling factor for delay thresholds (Delta_i)
    ...
    # Scale delay and error rate thresholds
...
def extract_metrics(optimizer, solution, data, psi_delay, psi_error, solve_time):
    ...
    Args:
        ...
        psi_delay: Delay threshold scaling factor
        psi_error: Error threshold scaling factor
    ...
    # Calculate delay constraint utilization per query type
    delay_utilization = {}
    for i in range(d.I):
        total_delay = 0
        for j in range(d.J):
            for k in range(d.K):
                x_val = solution['x'].get((i, j, k), 0)
                if x_val > 0:
                    total_delay += d.d_bar[i, j, k] * (d.h[i] + d.f[i]) * x_val
        delay_utilization[i] = total_delay / data.Delta_i[i] if data.Delta_i[i] > 0 else 0

    avg_delay_utilization = np.mean(list(delay_utilization.values()))
    max_delay_utilization = max(delay_utilization.values()) if delay_utilization else 0

    # Calculate error rate constraint utilization per query type
    error_utilization = {}
    for i in range(d.I):
        total_error = 0
        for j in range(d.J):
            for k in range(d.K):
                x_val = solution['x'].get((i, j, k), 0)
                if x_val > 0:
                    total_error += d.e_bar[i, j, k] * x_val
        error_utilization[i] = total_error / data.epsilon[i] if data.epsilon[i] > 0 else 0

    avg_error_utilization = np.mean(list(error_utilization.values()))
    max_error_utilization = max(error_utilization.values()) if error_utilization else 0
    ...
    return {
        ...
        'avg_delay': avg_delay,
        'avg_error': avg_error,
        'avg_delay_utilization': avg_delay_utilization,
        'max_delay_utilization': max_delay_utilization,
        'avg_error_utilization': avg_error_utilization,
        'max_error_utilization': max_error_utilization,

        # Scaled parameters
        'scaled_Delta_i_mean': np.mean(data.Delta_i),
...
def main():
    ...
    print("="*80)
    print("SENSITIVITY ANALYSIS: DELAY THRESHOLD & ERROR RATE THRESHOLD")
    print("="*80)
    ...
    psi_delay_values = [0.01, 0.03, 0.05, 0.07, 0.09]
    psi_error_values = [0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4]

    print("\n" + "="*80)
    print("SENSITIVITY ANALYSIS PARAMETERS")
    print("="*80)
    print(f"Base delay thresholds (Delta_i): {base_data.Delta_i}")
    print(f"Base error rate thresholds (epsilon): {base_data.epsilon}")
    print(f"\nDelay threshold scaling factors (psi_delay): {psi_delay_values}")
    print(f"Error rate threshold scaling factors (psi_error): {psi_error_values}")
    print(f"Total scenarios: {len(psi_delay_values)} × {len(psi_error_values)} = {len(psi_delay_values) * len(psi_error_values)}")
    ...
    for psi_delay in psi_delay_values:
        for psi_error in psi_error_values:
            counter += 1
            print(f"\n[{counter}/{total}] psi_delay={psi_delay:.1f}, psi_error={psi_error:.1f}")

            result = run_sensitivity_analysis(psi_delay, psi_error, base_data, time_limit=300, mip_gap=0.01)
    ...
    if len(df_success) > 0:
        ...
        csv_filename = os.path.join(output_dir, f'sensitivity_delay_error_threshold_{timestamp}.csv')
...
def create_visualizations(df, output_dir, timestamp):
    ...
    # Total Cost Heatmap
    cost_pivot = df.pivot_table(values='total_cost', index='psi_delay', columns='psi_error')
    ...
    axes[0, 0].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # Rental Cost Heatmap
    rental_pivot = df.pivot_table(values='C1_rental', index='psi_delay', columns='psi_error')
    ...
    axes[0, 1].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # Unmet Demand Cost Heatmap
    unmet_pivot = df.pivot_table(values='C4_unmet', index='psi_delay', columns='psi_error')
    ...
    axes[1, 0].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # Robust Delay Cost Heatmap
    robust_pivot = df.pivot_table(values='C3_robust_delay', index='psi_delay', columns='psi_error')
    sns.heatmap(robust_pivot, annot=True, fmt='.0f', cmap='Oranges', ax=axes[1, 1],
                cbar_kws={'label': 'Robust Delay Cost ($)'})
    axes[1, 1].set_title('Robust Delay Penalty (C3)', fontweight='bold', fontsize=12)
    axes[1, 1].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[1, 1].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'heatmaps_delay_error_threshold_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: heatmaps_delay_error_threshold_{timestamp}.png")
    plt.close()

    # Figure 2: Cost Component Breakdown
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))

    # Line plot for fixed delay threshold scaling
    for psi_delay in sorted(df['psi_delay'].unique())[::2]:  # Show every other value
        subset = df[df['psi_delay'] == psi_delay]
        axes[0].plot(subset['psi_error'], subset['total_cost'],
                    marker='o', label=f'ψ_delay={psi_delay:.1f}', linewidth=2)

    axes[0].set_xlabel('Error Threshold Scaling (ψ_error)', fontsize=11)
    axes[0].set_ylabel('Total Cost ($)', fontsize=11)
    axes[0].set_title('Total Cost vs Error Threshold\n(Different Delay Thresholds)', fontweight='bold')
    axes[0].legend(title='Delay Scaling', fontsize=9)
    axes[0].grid(True, alpha=0.3)

    # Line plot for fixed error threshold scaling
    for psi_error in sorted(df['psi_error'].unique())[::2]:  # Show every other value
        subset = df[df['psi_error'] == psi_error]
        axes[1].plot(subset['psi_delay'], subset['total_cost'],
                    marker='s', label=f'ψ_error={psi_error:.1f}', linewidth=2)

    axes[1].set_xlabel('Delay Threshold Scaling (ψ_delay)', fontsize=11)
    axes[1].set_ylabel('Total Cost ($)', fontsize=11)
    axes[1].set_title('Total Cost vs Delay Threshold\n(Different Error Thresholds)', fontweight='bold')
    axes[1].legend(title='Error Scaling', fontsize=9)
    axes[1].grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'cost_trends_delay_error_threshold_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: cost_trends_delay_error_threshold_{timestamp}.png")
    plt.close()
    ...
    x_labels = [f"ψ_d={row['psi_delay']:.1f}\nψ_e={row['psi_error']:.1f}"
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'cost_breakdown_delay_error_threshold_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: cost_breakdown_delay_error_threshold_{timestamp}.png")
    plt.close()

    # Figure 4: Performance Metrics Heatmaps
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))

    # Total GPUs Heatmap
    gpus_pivot = df.pivot_table(values='total_gpus', index='psi_delay', columns='psi_error')
    sns.heatmap(gpus_pivot, annot=True, fmt='.0f', cmap='Purples', ax=axes[0, 0],
                cbar_kws={'label': 'Total GPUs'})
    axes[0, 0].set_title('Total GPU Count', fontweight='bold', fontsize=12)
    axes[0, 0].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[0, 0].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # Unmet Demand Fraction Heatmap
    unmet_frac_pivot = df.pivot_table(values='total_unmet_fraction', index='psi_delay', columns='psi_error')
    sns.heatmap(unmet_frac_pivot, annot=True, fmt='.3f', cmap='Reds', ax=axes[0, 1],
                cbar_kws={'label': 'Unmet Demand Fraction'})
    axes[0, 1].set_title('Unmet Demand Fraction', fontweight='bold', fontsize=12)
    axes[0, 1].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[0, 1].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # Average Delay Heatmap
    delay_pivot = df.pivot_table(values='avg_delay', index='psi_delay', columns='psi_error')
    sns.heatmap(delay_pivot, annot=True, fmt='.1f', cmap='YlOrBr', ax=axes[1, 0],
                cbar_kws={'label': 'Average Delay'})
    axes[1, 0].set_title('Average Delay', fontweight='bold', fontsize=12)
    axes[1, 0].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[1, 0].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # Average Error Rate Heatmap
    error_pivot = df.pivot_table(values='avg_error', index='psi_delay', columns='psi_error')
    sns.heatmap(error_pivot, annot=True, fmt='.4f', cmap='RdPu', ax=axes[1, 1],
                cbar_kws={'label': 'Average Error Rate'})
    axes[1, 1].set_title('Average Error Rate', fontweight='bold', fontsize=12)
    axes[1, 1].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[1, 1].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'performance_heatmaps_delay_error_threshold_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: performance_heatmaps_delay_error_threshold_{timestamp}.png")
    plt.close()

    # Figure 5: Constraint Utilization Analysis
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))

    # Delay utilization vs error threshold scaling
    for psi_delay in sorted(df['psi_delay'].unique())[::2]:
        subset = df[df['psi_delay'] == psi_delay]
        axes[0].plot(subset['psi_error'], subset['avg_delay_utilization'],
                    marker='o', label=f'ψ_delay={psi_delay:.1f}', linewidth=2)

    axes[0].set_xlabel('Error Threshold Scaling (ψ_error)', fontsize=11)
    axes[0].set_ylabel('Average Delay Utilization', fontsize=11)
    axes[0].set_title('Delay Utilization vs Error Threshold\n(Different Delay Thresholds)', fontweight='bold')
    axes[0].legend(title='Delay Scaling', fontsize=9)
    axes[0].grid(True, alpha=0.3)
    axes[0].axhline(y=1.0, color='r', linestyle='--', label='Full Utilization', alpha=0.5)

    # Error utilization vs delay threshold scaling
    for psi_error in sorted(df['psi_error'].unique())[::2]:
        subset = df[df['psi_error'] == psi_error]
        axes[1].plot(subset['psi_delay'], subset['avg_error_utilization'],
                    marker='s', label=f'ψ_error={psi_error:.1f}', linewidth=2)

    axes[1].set_xlabel('Delay Threshold Scaling (ψ_delay)', fontsize=11)
    axes[1].set_ylabel('Average Error Utilization', fontsize=11)
    axes[1].set_title('Error Utilization vs Delay Threshold\n(Different Error Thresholds)', fontweight='bold')
    axes[1].legend(title='Error Scaling', fontsize=9)
    axes[1].grid(True, alpha=0.3)
    axes[1].axhline(y=1.0, color='r', linestyle='--', label='Full Utilization', alpha=0.5)

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'utilization_analysis_delay_error_threshold_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: utilization_analysis_delay_error_threshold_{timestamp}.png")
    plt.close()

    # Figure 6: Cost Percentage Analysis
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))

    # C1 percentage heatmap
    c1_pct_pivot = df.pivot_table(values='C1_pct', index='psi_delay', columns='psi_error')
    sns.heatmap(c1_pct_pivot, annot=True, fmt='.1f', cmap='Blues', ax=axes[0, 0],
                cbar_kws={'label': 'C1 Percentage (%)'})
    axes[0, 0].set_title('GPU Rental Cost Percentage', fontweight='bold', fontsize=12)
    axes[0, 0].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[0, 0].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # C2 percentage heatmap
    c2_pct_pivot = df.pivot_table(values='C2_pct', index='psi_delay', columns='psi_error')
    sns.heatmap(c2_pct_pivot, annot=True, fmt='.1f', cmap='Greens', ax=axes[0, 1],
                cbar_kws={'label': 'C2 Percentage (%)'})
    axes[0, 1].set_title('Storage Cost Percentage', fontweight='bold', fontsize=12)
    axes[0, 1].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[0, 1].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # C3 percentage heatmap
    c3_pct_pivot = df.pivot_table(values='C3_pct', index='psi_delay', columns='psi_error')
    sns.heatmap(c3_pct_pivot, annot=True, fmt='.1f', cmap='Oranges', ax=axes[1, 0],
                cbar_kws={'label': 'C3 Percentage (%)'})
    axes[1, 0].set_title('Robust Delay Cost Percentage', fontweight='bold', fontsize=12)
    axes[1, 0].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[1, 0].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    # C4 percentage heatmap
    c4_pct_pivot = df.pivot_table(values='C4_pct', index='psi_delay', columns='psi_error')
    sns.heatmap(c4_pct_pivot, annot=True, fmt='.1f', cmap='Reds', ax=axes[1, 1],
                cbar_kws={'label': 'C4 Percentage (%)'})
    axes[1, 1].set_title('Unmet Demand Cost Percentage', fontweight='bold', fontsize=12)
    axes[1, 1].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[1, 1].set_ylabel('Delay Threshold Scaling (ψ_delay)')

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'cost_percentage_heatmaps_delay_error_threshold_{timestamp}.png'), dpi=300, bbox_inches='tight')
    print(f"  Saved: cost_percentage_heatmaps_delay_error_threshold_{timestamp}.png")
    plt.close()
    ...
    df_sorted = df.sort_values(['psi_delay', 'psi_error'])
    ...
    # Create labels showing both memory/delay capacity and error threshold
    x_labels = [f"ψ_d={row['psi_delay']:.2f}\nψ_e={row['psi_error']:.1f}"
                for _, row in selected_scenarios.iterrows()]
    ...
    # Formatting
    ax.set_xlabel('Memory/Delay Capacity (ψ_d) & Error Threshold (ψ_e)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Number of Configurations', fontsize=12, fontweight='bold')
    ax.set_title('Tensor Parallelism Distribution Across Memory and Error Scenarios',
    ...
    tp_pivot = df.pivot_table(values='avg_tp_degree', index='psi_delay', columns='psi_error')
    ...
    axes[0, 0].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[0, 0].set_ylabel('Memory/Delay Capacity (ψ_delay)')

    # TP=1 count heatmap
    tp1_pivot = df.pivot_table(values='tp_1_count', index='psi_delay', columns='psi_error')
    ...
    axes[0, 1].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[0, 1].set_ylabel('Memory/Delay Capacity (ψ_delay)')

    # TP=2 count heatmap
    tp2_pivot = df.pivot_table(values='tp_2_count', index='psi_delay', columns='psi_error')
    ...
    axes[0, 2].set_ylabel('Memory/Delay Capacity (ψ_delay)')

    # TP=4 count heatmap
    tp4_pivot = df.pivot_table(values='tp_4_count', index='psi_delay', columns='psi_error')
    ...
    axes[1, 0].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[1, 0].set_ylabel('Memory/Delay Capacity (ψ_delay)')

    # TP=8 count heatmap
    tp8_pivot = df.pivot_table(values='tp_8_count', index='psi_delay', columns='psi_error')
    ...
    axes[1, 1].set_xlabel('Error Threshold Scaling (ψ_error)')
    axes[1, 1].set_ylabel('Memory/Delay Capacity (ψ_delay)')

    # Total TP configurations heatmap
    tp_total_pivot = df.pivot_table(values='num_tp_configs', index='psi_delay', columns='psi_error')
    ...
    axes[1, 2].set_ylabel('Memory/Delay Capacity (ψ_delay)')
