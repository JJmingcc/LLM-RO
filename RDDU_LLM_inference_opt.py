...
class LLMInferenceData:
    ...
    d: np.ndarray  # Processing delays (I x J)
    e: np.ndarray  # Error rates (I x J)
    d_bar: np.ndarray  # Nominal processing delays (I x J)
    d_hat: np.ndarray  # Processing delay deviations (I x J)
    e_bar: np.ndarray  # Nominal error rates (I x J)
    e_hat: np.ndarray  # Error rate deviations (I x J)
    Gamma_d: int  # Uncertainty budget for delays (scalar)
    Gamma_e: int  # Uncertainty budget for error rates (scalar)
    gamma_impact: np.ndarray  # Impact factor for delay uncertainty (J x K), values in [0, 1]
    error_impact: np.ndarray  # Impact factor for error rate uncertainty (J x K), values in [0, 1]
...
class DataGenerator:
    ...
    def generate(self) -> LLMInferenceData:
        ...
        d = self._generate_processing_delays(I, J)  # Returns array of size I x J
        e = self._generate_error_rates(I, J)  # Returns array of size I x J
        ...
        gamma_impact = self._generate_gamma_impact(J, K, model_names, gpu_tiers, P_gpu)
        error_impact = self._generate_error_impact(J, K, model_names, gpu_tiers, P_gpu)
        ...
        return LLMInferenceData(
            ...
            Gamma_d=Gamma_d, Gamma_e=Gamma_e, gamma_impact=gamma_impact, error_impact=error_impact, T_res=T_res
        )
    ...
    def _generate_processing_delays(self, I: int, J: int) -> np.ndarray:
        """
        Generate processing delay d[i,j] (ms per token) following RDDU_llm approach.
        ...
        Returns:
            2D array of size (I, J) with delay for each query-model combination
        """
    ...
    def _generate_error_rates(self, I: int, J: int) -> np.ndarray:
        """
        Generate error rate e[i,j] (fraction) following RDDU_llm approach.
        ...
        Returns:
            2D array of size (I, J) with error rate for each query-model combination
        """
    ...
    def _generate_gamma_impact(self, J: int, K: int, model_names: List[str], gpu_tiers: List[str], P_gpu: np.ndarray) -> np.ndarray:
        """
        Generate gamma_impact: delay reduction capability of GPU tier for each model.

        Higher gamma_impact = More capability to reduce processing delay
        - Higher-tier GPUs have higher impact (better performance)
        - Well-matched GPU-model pairs have highest impact

        Impact interpretation:
        - luxury (0.8-0.85): Powerful GPU, minimal delay
        - high (0.6-0.65): Strong GPU, good performance
        - medium (0.4-0.45): Moderate GPU, acceptable performance
        - low (0.2-0.25): Weak GPU, limited performance
        """
        model_complexity = {
            'Llama-3.2-1B': 'low',
            'Llama-3.2-3B': 'low',
    ...
    def _generate_error_impact(self, J: int, K: int, model_names: List[str], gpu_tiers: List[str], P_gpu: np.ndarray) -> np.ndarray:
        # More complex models have HIGHER error reduction impact on better hardware
        # 'high' = complex model (70B, 31B) - benefits more from better GPUs
        # 'low' = simple model (1B, 3B) - benefits less from better GPUs
        model_quality = {
            'Llama-3.2-1B': 'low',
            'Llama-3.2-3B': 'low',
            'Llama-3.1-8B': 'medium',
...
class LLMInferenceOptimizer:
    ...
    def build_and_solve_optimization_problem(self, time_limit: int = 300, mip_gap: float = 0.01):
        ...
        sigma_0 = self.model.addVars(d.I, d.J, vtype=GRB.CONTINUOUS, lb=0, name="sigma_0")
        sigma_1 = self.model.addVars(d.I, d.J, vtype=GRB.CONTINUOUS, lb=0, name="sigma_1")
        sigma_2 = self.model.addVars(d.I, d.J, vtype=GRB.CONTINUOUS, lb=0, name="sigma_2")

        # Auxiliary variables for RDDU (Routing Decision Dependent Uncertainty) constraints
        Z_0 = self.model.addVars(d.I, d.J, d.K, vtype=GRB.CONTINUOUS, lb=0, name="Z_0")  # Z_0 = sigma_0 * z
        Z_1 = self.model.addVars(d.I, d.J, d.K, vtype=GRB.CONTINUOUS, lb=0, name="Z_1")  # Z_1 = sigma_1 * z
        Z_2 = self.model.addVars(d.I, d.J, d.K, vtype=GRB.CONTINUOUS, lb=0, name="Z_2")  # Z_2 = sigma_2 * z
        ...
        self.vars = {
            ...
            'Z_0': Z_0, 'Z_1': Z_1, 'Z_2': Z_2, 'varrho': varrho}
        ...
        # Impact factors for routing-dependent uncertainty
        gamma_impact = d.gamma_impact
        error_impact = d.error_impact
        ...
        for i in range(d.I):
            self.model.addConstr(u[i] <= 0.01 * d.lambda_i[i], name=f"unmet_portion_restriction")
        ...
        self.model.addConstr(
            gp.quicksum(d.B[j] * z[i, j, k] for i in range(d.I) for j in range(d.J) for k in range(d.K)) <= d.C_storage, name="storage")
        ...
        for i in range(d.I):
            self.model.addConstr(
                gp.quicksum(d.d_bar[i, j] * (d.h[i] + d.f[i]) * x[i, j, k] for j in range(d.J) for k in range(d.K)) + d.Gamma_d * gp.quicksum(tau_0[i] for j in range(d.J) for k in range(d.K)) +
                gp.quicksum(sigma_0[i, j] for j in range(d.J)) - gp.quicksum(gamma_impact[j, k] * Z_0[i, j, k] for j in range(d.J) for k in range(d.K)) <= d.Delta_i[i] * gp.quicksum(n*w[j, k, n] for n in d.TP_degrees), name=f"robust_delay")

        # Robust error rate constraints (with routing-dependent uncertainty)
        for i in range(d.I):
            self.model.addConstr(
                gp.quicksum(d.e_bar[i, j] * x[i, j, k] for j in range(d.J) for k in range(d.K)) + d.Gamma_e * tau_1[i] + gp.quicksum(sigma_1[i, j] for j in range(d.J)) - gp.quicksum(error_impact[j, k] * Z_1[i, j, k] for j in range(d.J) for k in range(d.K)) <= d.epsilon[i], name=f"robust_error_rate_{i}")

        # Robust delay objective constraint
        self.model.addConstr(
            varrho >= gp.quicksum(d.Gamma_d * tau_2[i] for i in range(d.I)) + gp.quicksum(sigma_2[i, j] for i in range(d.I) for j in range(d.J)) - gp.quicksum(gamma_impact[j, k] * Z_2[i, j, k] for i in range(d.I) for j in range(d.J) for k in range(d.K)), name="robust_delay_obj")

        # robust error rate objective constraint
        # self.model.addConstr(
        #     zeta >= gp.quicksum(d.Gamma_e * tau_3[i] for i in range(d.I)) + gp.quicksum(sigma_3[i, j] for i in range(d.I) for j in range(d.J)) - gp.quicksum(error_impact[j, k] * Z_3[i, j, k] for i in range(d.I) for j in range(d.J) for k in range(d.K)), name="robust_error_rate_obj")
        ...
        for i in range(d.I):
            for j in range(d.J):
                for k in range(d.K):
                    ...
                    self.model.addConstr(tau_0[i] + sigma_0[i, j] >= d.d_hat[i, j] * (d.h[i] + d.f[i]) * x[i, j, k], name=f"dual_delay")

                    # Dual constraint for error rate
                    self.model.addConstr(tau_1[i] + sigma_1[i, j] >= d.e_hat[i, j] * x[i, j, k], name=f"dual_error_{i}_{j}_{k}")

                    # Dual constraint for delay objective
                    self.model.addConstr(tau_2[i] + sigma_2[i, j] >= d.d_hat[i, j] * d.rho[i] * d.lambda_i[i] * (d.h[i] + d.f[i]) * x[i, j, k], name=f"dual_delay_obj")

                    # # Dual constraint for error rate objective
                    # self.model.addConstr(tau_3[i] + sigma_3[i, j] >=  d.e_hat[i, j] * d.lambda_i[i] * x[i, j, k], name=f"dual_error_obj")

        # === LINEARIZATION CONSTRAINTS FOR RDDU ===

        # Linearization for Z_0 = sigma_0 * z (Big-M method)
        for i in range(d.I):
            for j in range(d.J):
                for k in range(d.K):
                    self.model.addConstr(Z_0[i, j, k] >= sigma_0[i, j] - d.BigM * (1 - z[i, j, k]))
                    self.model.addConstr(Z_0[i, j, k] <= sigma_0[i, j])
                    self.model.addConstr(Z_0[i, j, k] <= d.BigM * z[i, j, k])

        # Linearization for Z_1 = sigma_1 * z
        for i in range(d.I):
            for j in range(d.J):
                for k in range(d.K):
                    self.model.addConstr(Z_1[i, j, k] >= sigma_1[i, j] - d.BigM * (1 - z[i, j, k]))
                    self.model.addConstr(Z_1[i, j, k] <= sigma_1[i, j])
                    self.model.addConstr(Z_1[i, j, k] <= d.BigM * z[i, j, k])

        # Linearization for Z_2 = sigma_2 * z
        for i in range(d.I):
            for j in range(d.J):
                for k in range(d.K):
                    self.model.addConstr(Z_2[i, j, k] >= sigma_2[i, j] - d.BigM * (1 - z[i, j, k]))
                    self.model.addConstr(Z_2[i, j, k] <= sigma_2[i, j])
                    self.model.addConstr(Z_2[i, j, k] <= d.BigM * z[i, j, k])

        # # Linearization for Z_3 = sigma_2 * z
        # for i in range(d.I):
        #     for j in range(d.J):
        #         for k in range(d.K):
        #             self.model.addConstr(Z_3[i, j, k] >= sigma_3[i, j] - d.BigM * (1 - z[i, j, k]))
        #             self.model.addConstr(Z_3[i, j, k] <= sigma_3[i, j])
        #             self.model.addConstr(Z_3[i, j, k] <= d.BigM * z[i, j, k])
    ...
    def _display_constraint_analysis(self, solution: Dict):
        ...
        for i in range(d.I):
            ...
            unmet_threshold = 0.001 * d.lambda_i[i]
        ...
        for i in range(d.I):
            ...
            for j in range(d.J):
                delay_lhs += self.vars['sigma_0'][i, j].X
            for j in range(d.J):
                for k in range(d.K):
                    delay_lhs -= d.gamma_impact[j, k] * self.vars['Z_0'][i, j, k].X
        ...
        for i in range(d.I):
            ...
            for j in range(d.J):
                error_lhs += self.vars['sigma_1'][i, j].X
            for j in range(d.J):
                for k in range(d.K):
                    error_lhs -= d.error_impact[j, k] * self.vars['Z_1'][i, j, k].X
